...
- Mac MPS (M-серия): ~4 сек на фото 400×400 → 4×, **рекомендуется `--batch-size 1`**
  (батчинг на MPS медленнее single-image из-за padding и MPS backend ограничений)
- RTX 4090 (CUDA): ~100-150 ms одиночное фото, **с `--batch-size 16` ожидается ~30-50 фото/сек**
  (точный замер виртус сделает на калибровочном прогоне)
